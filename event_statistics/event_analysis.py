...
def spatial_correlation0(x, y, polarity, timestamp, spat_corr, timestamps, tau):
    if x >= l and x <= rf_size[0] - l - 1 and y >= l and y <= rf_size[1] - l - 1:
        spat_corr += 1 * (
            timestamp - timestamps[y - l : y + l + 1, x - l : x + l + 1] <= tau
        )  # P(On | On)
...
for tau in bins:
    with AedatFile(file_name) as f:
        ...
        count = 0
        for e in f["events"]:
            spatial_correlation0(
                e.x, e.y, e.polarity, e.timestamp, spat_corr, timestamps, tau
            )
            count += 1
    ...
    with AedatFile(file_name) as f:
        ...
        for e in f["events"]:
            temporal_correlation(
                e.x, e.y, e.polarity, e.timestamp, temp_corr, timestamps, tau
            )
            count += 1
