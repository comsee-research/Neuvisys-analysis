...
def launch_spinet(directory, n_iter):
    ...
    generate_networks(directory, network_params, neuron_params, pooling_neuron_params, n_iter)

def generate_networks(directory, network_params, neuron_params, pooling_neuron_params, n_iter):
...
def launch_neuvisys_rotation(network_path, event_file, rotation):
    for path in execute(["/home/thomas/neuvisys-dv/cmake-build-release/event-analysis", network_path, "rotation", event_file, str(rotation)]):
        print(path, end="")

def launch_neuvisys_multi_pass(network_path, event_file, nb_pass):
    for path in execute(["/home/thomas/neuvisys-dv/cmake-build-release/event-analysis", network_path, "multi-pass", event_file, str(nb_pass)]):
        print(path, end="")

def launch_neuvisys_stereo(network_path, left_file, right_file, nb_pass):
    for path in execute(["/home/thomas/neuvisys-dv/cmake-build-release/event-analysis", network_path, "stereo", left_file, right_file, str(nb_pass)]):
        print(path, end="")